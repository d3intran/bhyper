...
impl BinanceWsStream {
    pub fn spawn(cache: MarketDataCache) {
        tokio::spawn(async move {
            ...
            loop {
                info!("Connecting to Binance FAPI WebSocket stream at {}...", ws_url);
                match connect_async(ws_url).await {
                        ...
                        while let Some(msg_res) = ws_stream.next().await {
                            match msg_res {
                                Ok(tokio_tungstenite::tungstenite::Message::Text(text)) => {
                                    if let Ok(items) = serde_json::from_str::<Vec<BinanceWsMarkPriceItem>>(&text) {
                                        ...
                                        for item in items {
                                            ...
                                            let rate_8h = item.funding_rate.parse::<f64>().unwrap_or(0.0);
                    ...
                    Err(e) => {
                        error!("Failed to connect to Binance WebSocket: {:?}. Retrying in 3s...", e);
                    }
                }
