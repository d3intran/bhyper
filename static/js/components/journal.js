...
export async function fetchAndRenderJournal(filterType = '') {
  ...
  try {
    ...
    if (!res.entries || res.entries.length === 0) {
      tbody.innerHTML = `<tr><td colspan="7" class="text-center py-10 text-[var(--text-muted)]">暂无流水数据</td></tr>`;
      return;
    }
    ...
    tbody.innerHTML = res.entries.map(e => {
      ...
      if (typeName === 'Funding') {
        typeBadge = `<span class="px-2 py-0.2 rounded text-[10px] font-medium bg-cyan-500/10 text-cyan-500 border border-cyan-500/20">FUNDING</span>`;
        detail = `${d.exchange || ''} ${d.side || ''} (费率: ${(d.rate_bps || 0).toFixed(2)} bps)`;
        ...
        typeBadge = `<span class="px-2 py-0.2 rounded text-[10px] font-medium bg-emerald-500/10 text-emerald-500 border border-emerald-500/20">OPEN</span>`;
        ...
        pnlOrFunding = `<span class="text-slate-400">已完全对冲</span>`;
        fees = formatCurrency(d.total_open_fees_usd || 0, 4);
      } else if (typeName === 'CloseFill') {
        typeBadge = `<span class="px-2 py-0.2 rounded text-[10px] font-medium bg-rose-500/10 text-rose-500 border border-rose-500/20">CLOSE</span>`;
        detail = `HL: ${formatPrice(d.hyperliquid_exit_price)} / BN: ${formatPrice(d.binance_exit_price)} (基差: ${formatCurrency(d.gross_basis_pnl_usd || 0, 4)})`;
        ...
        typeBadge = `<span class="px-2 py-0.2 rounded text-[10px] font-medium bg-purple-500/10 text-purple-500 border border-purple-500/20">INTENT</span>`;
        detail = `${d.reason || '策略触发开仓决策'}`;
        ...
        typeBadge = `<span class="px-2 py-0.2 rounded text-[10px] font-medium bg-amber-500/10 text-amber-500 border border-amber-500/20">RISK</span>`;
        detail = `${d.event_type || ''}: ${d.details || ''} (${d.action_taken || ''})`;
        pnlOrFunding = `<span class="text-amber-500 font-medium">风控拦截</span>`;
      }
