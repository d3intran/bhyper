...
export async function fetchAndRenderJournal(filterType = '') {
  ...
  try {
    ...
    if (!res.entries || res.entries.length === 0) {
      tbody.innerHTML = `
        <tr>
          <td colspan="7" class="text-center py-12 text-ink-mute">
            <div class="flex flex-col items-center justify-center space-y-2">
              <i data-lucide="file-text" class="w-6 h-6 text-ink-mute opacity-60"></i>
    ...
    tbody.innerHTML = res.entries.map(e => {
      ...
      let typeBadge = `<span class="px-2 py-0.2 rounded text-2xs font-medium bg-subtle text-ink">${typeName}</span>`;
      let detail = '';
      let notional = '-';
      let pnlOrFunding = '-';
      let fees = '-';
      ...
      if (typeName === 'Funding') {
        typeBadge = `<span class="px-2 py-0.2 rounded text-2xs font-medium bg-cyan-500/10 text-cyan-500 border border-cyan-500/25">FUNDING</span>`;
        ...
        typeBadge = `<span class="px-2 py-0.2 rounded text-2xs font-medium bg-emerald-500/10 text-emerald-500 border border-emerald-500/25">OPEN</span>`;
        ...
        pnlOrFunding = `<span class="text-ink-mute">Fully Hedged</span>`;
        fees = formatCurrency(d.total_open_fees_usd || 0, 4);
      } else if (typeName === 'CloseFill') {
        typeBadge = `<span class="px-2 py-0.2 rounded text-2xs font-medium bg-rose-500/10 text-rose-500 border border-rose-500/25">CLOSE</span>`;
        ...
        typeBadge = `<span class="px-2 py-0.2 rounded text-2xs font-medium bg-subtle text-ink-soft border border-edge">INTENT</span>`;
        ...
        typeBadge = `<span class="px-2 py-0.2 rounded text-2xs font-medium bg-amber-500/10 text-amber-500 border border-amber-500/25">RISK</span>`;
      ...
      return `
        <tr>
          <td class="py-2.5 px-3.5 text-ink-mute">${time}</td>
          <td class="py-2.5 px-2">${typeBadge}</td>
          <td class="py-2.5 px-2 font-semibold text-ink">${d.symbol}</td>
          <td class="py-2.5 px-3 text-ink-soft">${detail}</td>
          <td class="py-2.5 px-2 text-right text-ink-mute">${notional}</td>
          <td class="py-2.5 px-2 text-right">${pnlOrFunding}</td>
          <td class="py-2.5 px-3 text-right ${fees.startsWith('$') && fees !== '$0.0000' ? 'text-rose-500' : 'text-ink-mute'}">${fees.startsWith('$') && fees !== '$0.0000' ? '-' + fees : fees}</td>
        </tr>
      `;
    }).join('');
